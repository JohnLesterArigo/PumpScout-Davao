...
class TrainedPriceForecastService {
  static const String _forecastApiUrl = String.fromEnvironment(
    'FORECAST_API_URL',
    defaultValue: 'https://pumpscout-davao.onrender.com/predict',
  );
  ...
  static Future<FuelPriceForecast?> forecastFuelPrice({
    ...
    final productType = _productTypeForForecastFuel(station, fuelType);

    final apiForecast = await _forecastFuelPriceFromApi(
      points: points,
      station: station,
      fuelType: fuelType,
    ...
    if (apiForecast != null) return apiForecast;
  ...
  static Future<FuelPriceForecast?> _forecastFuelPriceFromApi({
    required List<_ForecastPoint> points,
    required StationMarkerDetails station,
    required String fuelType,
    required String productType,
    required FuelPriceBand band,
  }) async {
    if (_forecastApiUrl.trim().isEmpty) return null;
    ...
    try {
      ...
      final predictedPrice = _jsonDouble(decoded['predictedPrice']);
      if (predictedPrice == null) return null;

      final confidencePercent = _jsonDouble(decoded['confidencePercent']);
      final predictedAt = DateTime.tryParse('${decoded['predictedAt']}') ??
          latest.date.add(const Duration(days: 7));
      final confidence = ((confidencePercent ?? 65) / 100)
          .clamp(0.0, 1.0)
          .toDouble();
      final clampedPrice = predictedPrice.clamp(band.min, band.max).toDouble();
      ...
      return FuelPriceForecast(
        ...
        predictedAt: predictedAt,
        slopePerDay: (clampedPrice - latest.price) / 7,
        confidence: confidence,
        modelVersion: _stringField(
          decoded,
          'modelVersion',
        ...
        methodLabel: 'Forecast generated by Flask API on Render.',
      );
    } catch (error) {
      debugPrint('Forecast API unavailable, using local model: $error');
      return null;
    }
  }
}
